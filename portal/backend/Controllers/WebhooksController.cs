...
public class WebhooksController : ControllerBase
{
    ...
    public WebhooksController(PortalDbContext context, IWebhookSignatureValidator signatureValidator, IConfiguration configuration, ILogger<WebhooksController> logger, IEmailService emailService, IModuleOwnershipService moduleOwnershipService)
    {
        _context = context;
    ...
    public async Task<IActionResult> HandleNpmWebhook()
    {
        ...
        try
        {
            ...
            var mapping = await _context.PackageRegistryMappings.Include(m => m.Module).FirstOrDefaultAsync(m => m.RegistryType == "npm" && m.PackageName == payload.Name);
            if (mapping == null)
            {
                _logger.LogWarning("No module mapping found for npm package: {Package}", payload.Name);
                var errorResponse = new
                {
                    error = "Package not registered",
            ...
            var existingVersion = await _context.ModuleVersions.FirstOrDefaultAsync(mv => mv.ModuleId == mapping.ModuleId && mv.Version == payload.Version);
    ...
    public IActionResult TestWebhook()
    {
        ...
        return Ok(new { npmWebhookConfigured = !string.IsNullOrEmpty(npmSecret), nugetWebhookConfigured = !string.IsNullOrEmpty(nugetSecret), message = "Webhook endpoints are configured" });
    }
    ...
    public async Task<IActionResult> NuGetWebhook()
    {
        ...
        try
        {
            ...
            var mapping = await _context.PackageRegistryMappings.Include(m => m.Module).FirstOrDefaultAsync(m => m.RegistryType == "nuget" && m.PackageName == payload.PackageId);
            if (mapping == null)
            {
    ...
    public async Task<IActionResult> GetWebhookHistory([FromQuery] string? registryType = null, [FromQuery] string? packageName = null, [FromQuery] DateTime? startDate = null, [FromQuery] DateTime? endDate = null, [FromQuery] int? statusCode = null, [FromQuery] int page = 1, [FromQuery] int pageSize = 50)
    {
        try
        {
            ...
            var webhooks = await query.OrderByDescending(w => w.CreatedAt).Skip((page - 1) * pageSize).Take(pageSize).Select(w => new { w.Id, w.Endpoint, w.RegistryType, w.EventType, w.PackageName, w.PackageVersion, w.IpAddress, w.StatusCode, w.SignatureValid, w.ProcessingTimeMs, w.CreatedAt }).ToListAsync();
            return Ok(new { total, page, pageSize, totalPages = (int)Math.Ceiling(total / (double)pageSize), webhooks });
        }
        catch (Exception ex)
        {
